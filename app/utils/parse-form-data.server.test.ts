...
describe('validateFormData()', () => {
  ...
  test('given a schema and invalid values: throws a bad request response with all errors in an error object', async () => {
    expect.assertions(1);
    ...
    try {
      ...
      if (error instanceof Response) {
        ...
        const expected = {
          errors: {
            age: ['age-must-be-int', 'minimum-age-is-18'],
            email: ['invalid-email'],
          },
          message: 'Bad Request',
        };
  ...
  test('given a violation of the root schema: throws a bad request response a form errors', async () => {
    ...
    try {
      ...
      if (error instanceof Response) {
        const actual = await error.json();
        const expected = {
          errors: { form: ['Expected object, received null'] },
          message: 'Bad Request',
        };
...
describe('withValidatedFormData()', () => {
  test('given Zod schema and a request: validates the request form data against the Zod schema', async () => {
    ...
    const expected = { request, foo: 'bar', ...values };

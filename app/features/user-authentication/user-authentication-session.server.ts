...
const USER_AUTHENTICATION_SESSION_NAME = '__user-authentication-session';
const ONE_YEAR_IN_SECONDS = 60 * 60 * 24 * 365;

const userAuthenticationSessionStorage = createCookieSessionStorage({
  cookie: {
    httpOnly: true,
    maxAge: 0,
...
export async function createCookieForUserAuthSession({
  ...
  return sessionStorage.commitSession(session, { maxAge: ONE_YEAR_IN_SECONDS });
}
